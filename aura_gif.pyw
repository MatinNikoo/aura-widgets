...
GIF_DIR = os.path.join(APP_DIR, "gifs")
os.makedirs(GIF_DIR, exist_ok=True)
RUN_NAME = "AuraGifWidget"
...
def main():
    ...
    lock = QLockFile(os.path.join(tempfile.gettempdir(), "aura_gif_widget.lock"))
    if not lock.tryLock(100):
        return
    settings = QSettings("Aura", "Gif")
    ...
    class Widget(QWidget):
        def __init__(self):
            ...
            if isinstance(pos, QPoint):
                ...
                self.move(g.right() - self.width() - 20, g.bottom() - self.height() - 20)
